...
class LocalGlobalRegistration(nn.Module):
    ...
    def local_to_global_registration(self, ref_knn_points, src_knn_points, score_mat, corr_mat,ref_knn_colors=None,src_knn_colors=None):
        ...
        if ref_knn_colors is not None:
            global_ref_corr_colors = ref_knn_points[batch_indices, ref_indices]
            global_src_corr_colors = src_knn_points[batch_indices, src_indices]
        ...
        for _ in range(self.num_refinement_steps - 1):
            ...
            estimated_transform = self.procrustes(src_corr_points, ref_corr_points, cur_corr_scores)
        if ref_knn_colors is not None:
            return global_ref_corr_points, global_src_corr_points, global_corr_scores, estimated_transform,global_ref_corr_colors,global_src_corr_colors
        else:
    ...
    def forward(
        ...
        if ref_knn_colors is not None:
            ref_corr_points, src_corr_points, corr_scores, estimated_transform, global_ref_corr_colors,global_src_corr_colors = self.local_to_global_registration(
                ref_knn_points, src_knn_points, score_mat, corr_mat,ref_knn_colors,src_knn_colors
            )
            return ref_corr_points, src_corr_points, corr_scores, estimated_transform, global_ref_corr_colors,global_src_corr_colors
        else:
