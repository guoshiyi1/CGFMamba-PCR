...
class CGFMamba(nn.Module):
    def __init__(self, config, **kwargs):
        super(CGFMamba, self).__init__()
        self.config = config
        ...
        if config.flag is True:
            ...
            self.in_proj = nn.Linear(input_dim, self.D)
    ...
    def forward(self, center,neighborhood,xyz=None):

        if len(neighborhood.shape) == 4:
            group_input_tokens = self.encoder(neighborhood)  # B G N
            pos = self.pos_embed(center)# 
            B, N, _ = center.shape
        else:
            B, N, C = center.shape
            # group_input_tokens = self.in_proj(center)
            group_input_tokens = self.in_proj(center)
            if self.color:
                pos = self.pos_embed(neighborhood[:,:N,:])
                color = self.color_embed(neighborhood[:,N:,:])
                pos = torch.cat((pos,color),dim=2)
            elif self.config.geopos:
                pos = neighborhood
            else:
                pos = self.pos_embed(neighborhood)

        x = group_input_tokens
        x = self.drop_out(x)
        x = self.blocks(x, pos)
        x = x+group_input_tokens
